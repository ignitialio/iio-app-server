...
class APIGateway extends Module {
  constructor(options) {
    super({
      name: 'apigateway',
      ...options
    ...
    this._gateway._init().then(() => {
      ...
      this._gateway.on('heartbeat', message => {
        this.$app.ws._io.emit('service:heartbeat:event', message)
      })
  ...
  async _createBridge(serviceName, serviceInfo) {
    try {
      ...
      this._bridges[serviceName] = this.$app.instantiateModule(serviceName, serviceInfo)
      ...
      for (let method of serviceInfo.methods) {
        ...
        this._bridges[serviceName][method] = this._gateway.api[serviceName][method]
      }

      // register full service as a gateway bridged object
      this._bridges[serviceName]._register()

      this.logger.info('gateway module registered new service module [%s]',
        serviceName)

      console.log('gateway module registered new service module [%s]',
        serviceName)
    } catch (err) {
    ...
    for (let s in this._bridges) {
      if (subset) {
        if (subset.indexOf(s) !== -1) {
          ...
          this.logger.warn('destroy [%s] bridge from module gateway [%s]', s)
        }
      } else {
        ...
        this.logger.warn('destroy [%s] bridge from gateway module', s)
      }
    }
  }
